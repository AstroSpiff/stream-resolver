...
def _mf():
    d = _load_settings()
    mf = (d.get("mediaflow_url") or "").rstrip("/")
    pwd = d.get("api_password") or ""
    if not mf or not pwd:
        raise ResolverError("mediaflow_misconfigured")
    return mf, pwd

def _mf_proxy_url(kind: str, mf: str, pwd: str, stream_url: str, headers: dict) -> str:
    base = f"{mf}/proxy/hls/manifest.m3u8" if kind=="hls" else f"{mf}/proxy/mpd/playlist.m3u8"
    params = [
        ("d", stream_url),
        ("api_password", pwd),
    ]
    # inoltra header utili
    mapping = {
        "referer":"h_referer", "origin":"h_origin",
        "user-agent":"h_user-agent", "cookie":"h_cookie",
        "range": "h_range",
    }
    ...
    q = "&".join(f"{k}={_enc(v)}" for k,v in params if v)
    return f"{base}?{q}"

def _is_hls_or_mpd(url: str) -> str:
    l = (url or "").lower().split("?")[0]
    if l.endswith(".m3u8"): return "hls"
    if l.endswith(".mpd"):  return "mpd"
    return ""

# --- (legacy) VixSrc/VixCloud two-pass ---------------------------------------
# Lasciato intatto ma non usato per VixSrc: ora usiamo il fastpath a singolo hop.
def resolve_vix_via_mediaflow(page_url: str) -> dict:
    """
    Passo 1: extractor JSON (senza redirect) per ottenere stream_url + headers
    Passo 2: costruzione URL proxy HLS/MPD->HLS ffmpeg-friendly con header inoltrati
    """
    import httpx
    mf, pwd = _mf()

    # 1) extractor (NO redirect_stream)
    ext = f"{mf}/extractor/video?host=VixCloud&api_password={_enc(pwd)}&d={_enc(page_url)}"
    ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0 Safari/537.36"
    with httpx.Client(follow_redirects=True, timeout=40) as s:
        r = s.get(ext, headers={"User-Agent": ua, "Referer": page_url, "Origin": "https://vixsrc.to"})
        if r.status_code != 200:
            raise ResolverError(f"mf_extractor_{r.status_code}")
        data = r.json() if r.headers.get("content-type","").startswith("application/json") else {}
    res = data.get("result") or data
    stream_url = (res.get("stream_url") or res.get("url") or "").strip()
    if not stream_url:
        redir = f"{mf}/extractor/video?host=VixCloud&redirect_stream=true&api_password={_enc(pwd)}&d={_enc(page_url)}"
        return {"ok": True, "resolvedUrl": redir, "meta": {"resolver": "MediaFlow.VixCloud:fallback"}}

    headers = res.get("headers") or {}
    headers.setdefault("referer", page_url)
    headers.setdefault("origin", "https://vixsrc.to")
    headers.setdefault("user-agent", ua)

    kind = _is_hls_or_mpd(stream_url) or "hls"
    final_url = _mf_proxy_url("hls" if kind=="hls" else "mpd", mf, pwd, stream_url, headers)
    return {"ok": True, "resolvedUrl": final_url, "meta": {"resolver": "MediaFlow.VixCloud"}}
